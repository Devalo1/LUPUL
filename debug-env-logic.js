...
const mockEnv = {
  ...
  NETOPIA_LIVE_API_KEY: 'VfjsAdVjct7hQkMXRHKlimzmGGUHztw1e-C1PmvUoBlxkHs05BeWPpx0SXgV'
};
...
function debugEnvironmentDetection() {
  ...
  console.log('NETOPIA_LIVE_SIGNATURE present:', !!mockEnv.NETOPIA_LIVE_SIGNATURE);
  console.log('NETOPIA_LIVE_API_KEY present:', !!mockEnv.NETOPIA_LIVE_API_KEY);
  
  // Test cu paymentData.live = undefined (default)
  const paymentData1 = {};
  const useLive1 = paymentData1.live === true || (isProduction && hasLiveSignature);
  ...
  console.log('(isProduction && hasLiveSignature):', (isProduction && hasLiveSignature));
  console.log('useLive:', useLive1);
  
  // Test cu paymentData.live = true
  const paymentData2 = { live: true };
  const useLive2 = paymentData2.live === true || (isProduction && hasLiveSignature);
  ...
  console.log('\n📋 Expected Result:');
  console.log('Should use LIVE in production:', (isProduction && hasLiveSignature));
}
