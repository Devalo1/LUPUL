...
export const httpEventRegistration = functionsV1.https.onRequest((req, res) => {
  ...
  return corsHandler(req, res, async () => {
    ...
    try {
      const data = req.body as EventRegistrationData;
      const {
        registrationId,
        eventId,
        eventTitle,
...
export const sendEventRegistrationNotification = onCall(async (request) => {
  ...
  try {
    ...
    const data = request.data as unknown as EventRegistrationData;
    const {
      registrationId,
      eventId,
      eventTitle,
...
export const uploadProfilePhoto = onCall(async (request) => {
  ...
  if (!request.auth) {
    throw new HttpsError(
      "unauthenticated",
      "Trebuie să fiți autentificat pentru a utiliza această funcție."
    );
  }

  try {
    const { photoBase64, userId } = request.data as { photoBase64: string; userId: string };
    ...
    if (request.auth.uid !== userId) {
      ...
      if (!customClaims.admin) {
        throw new HttpsError(
          "permission-denied",
          "Nu aveți permisiunea de a actualiza fotografia altui utilizator."
        );
      }
    }
