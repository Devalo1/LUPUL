...
class UserProfileManager {
    ...
    if (!profileDoc.exists) {
      const initialProfile = {
        ...
        profile: {
          ...
          // Personalitate și comportament (cu tipar de vorbire și preferințe)
          personalityTraits: [],
  ...
  // Adaugă o plăcere
  async addPleasure(pleasure) {
    await this.userRef.update({
      "profile.pleasures": admin.firestore.FieldValue.arrayUnion(pleasure),
      "metadata.lastUpdated": admin.firestore.FieldValue.serverTimestamp(),
    });
  }
  ...
  async addDesire(desire) {
    await this.userRef.update({
      "profile.desires": admin.firestore.FieldValue.arrayUnion(desire),
      "metadata.lastUpdated": admin.firestore.FieldValue.serverTimestamp(),
    });
  }

  // Adaugă o condiție de sănătate
  async addHealthCondition(condition) {
    await this.userRef.update({
      "profile.healthConditions":
        admin.firestore.FieldValue.arrayUnion(condition),
      "metadata.lastUpdated": admin.firestore.FieldValue.serverTimestamp(),
    });
  }
  ...
  async addMedication(medication) {
    await this.userRef.update({
      "profile.medications": admin.firestore.FieldValue.arrayUnion(medication),
      "metadata.lastUpdated": admin.firestore.FieldValue.serverTimestamp(),
    });
  }
  ...
  async addConcern(concern) {
    await this.userRef.update({
      "profile.concerns": admin.firestore.FieldValue.arrayUnion(concern),
      "metadata.lastUpdated": admin.firestore.FieldValue.serverTimestamp(),
    });
  }
  ...
  async generateContextForAI() {
    ...
    // Trăsături de personalitate și comportament
  ...
  async analyzeProfileGaps() {
    ...
    // Verifică preferințe comunicare și comportament
    ...
    if (!p.speechPattern) {
      missingInfo.push({
        type: "speechPattern",
        question:
          "Cum preferi să comunici - mai direct, mai diplomatic sau în alt fel?",
      });
    }
    ...
    if (!p.behaviorPattern) {
      missingInfo.push({
        type: "behavior",
        question:
          "Cum te-ai descrie ca personalitate - mai comunicativ, mai rezervat sau cum?",
      });
    }
    ...
    if (!p.desires || p.desires.length === 0) {
      missingInfo.push({
        type: "desires",
        question: "Ce îți dorești cel mai mult să realizezi sau să faci?",
      });
    }
    ...
    // Verifică preocupări generale
    if (p.concerns.length === 0) {
      missingInfo.push({
        type: "concerns",
        question:
          "Ce te preocupă cel mai mult în această perioadă? Ce îți face griji?",
      });
    }
...
function extractInfoFromMessage(message) {
  ...
  for (const pattern of goalPatterns) {
    ...
    if (match && match[1]) {
      if (
        pattern.source.includes("doresc") ||
        pattern.source.includes("plăcea") ||
        pattern.source.includes("visez")
      ) {
  ...
  // Extragere plăceri și preferințe
  const pleasurePatterns = [
    ...
    /îmi aduce plăcere ([a-zA-ZăâîșțĂÂÎȘȚ\s]+)/i,
    /savurez ([a-zA-ZăâîșțĂÂÎȘȚ\s]+)/i,
  ];
  ...
  // Extragere informații de sănătate
  ...
  // Extragere medicamentație
  const medicationPatterns = [
    /iau ([a-zA-ZăâîșțĂÂÎȘȚ\s]*(?:pastile|medicamente|tratament)[a-zA-ZăâîșțĂÂÎȘȚ\s]*)/i,
    /iau ([a-zA-ZăâîșțĂÂÎȘȚ\s]*(?:aspirină|paracetamol|ibuprofen|antibiotic)[a-zA-ZăâîșțĂÂÎȘȚ\s]*)/i,
  ...
  // Detecție stil comunicare/comportament
  if (
