...
export const handler = async (event, context) => {
  try {
    ...
    const envVars = {
      NETOPIA_LIVE_SIGNATURE: process.env.NETOPIA_LIVE_SIGNATURE ? "SET (length: " + process.env.NETOPIA_LIVE_SIGNATURE.length + ")" : "NOT SET",
      NETOPIA_LIVE_PUBLIC_KEY: process.env.NETOPIA_LIVE_PUBLIC_KEY ? "SET (length: " + process.env.NETOPIA_LIVE_PUBLIC_KEY.length + ")" : "NOT SET",
      NETOPIA_SANDBOX_PUBLIC_KEY: process.env.NETOPIA_SANDBOX_PUBLIC_KEY ? "SET" : "NOT SET",
      URL: process.env.URL || "NOT SET",
      NODE_ENV: process.env.NODE_ENV || "NOT SET",
    };

    // Determină modul de funcționare
    const isLive = process.env.NETOPIA_LIVE_SIGNATURE && 
