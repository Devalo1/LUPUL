...
import { UserRole, requestRoleChange, checkPendingRoleRequests, isUserSpecialist } from "../utils/userRoles";
...
const SpecialistRoleRequest: React.FC<SpecialistRoleRequestProps> = ({ 
  ...
  const [requestStatus, setRequestStatus] = useState<"none" | "success" | "error" | "existing">("none");
  ...
  const handleRequestRoleChange = async () => {
    ...
    try {
      ...
      // Convert specialization string to UserRole enum if needed
      const _roleValue = "specialist"; // or whatever the value was
      ...
      } else if (typeof result === "boolean" || (typeof result === "string" && result !== "existing")) {
      ...
      if (typeof result === "boolean" || (typeof result === "string" && result !== "existing")) {
        setTimeout(() => {
          setModalOpen(false);
          resetForm();
        }, 3000);
  ...
  if (isSpecialist || userRole === UserRole.SPECIALIST) {
    console.log("User is a specialist, not showing specialist role request form");
    return null;
  }
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-4">
                      ...
                      {requestStatus === "none" && (
                        <>
                          <div className="mb-4">
                            <label htmlFor="specializationCategory" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {requestStatus === "none" && (
                  <button
                    ...
                    className={`w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm ${
                      requestingRoleChange || !roleChangeReason.trim() || !specialization.trim() || !specializationCategory.trim() || !education.trim() || !yearsOfExperience || !certificateUrl.trim() ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                    onClick={handleRequestRoleChange}
                    disabled={requestingRoleChange || !roleChangeReason.trim() || !specialization.trim() || !specializationCategory.trim() || !education.trim() || !yearsOfExperience || !certificateUrl.trim()}
                  >
