...
const TokenRecoveryModal: React.FC = () => {
    ...
    if (auth.currentUser) {
      ...
      if (hasTokenProblems) {
        ...
        if (errorCount >= 2 && !tokenHealth.circuitBreakerActive) {
          recoveryLogger.warn(
            "Multiple erori de token detectate, afișăm modalul de recuperare"
          );
          setIsVisible(true);
        }
      } else {
      ...
      if (tokenHealth.circuitBreakerActive) {
        recoveryLogger.info(
          "Circuit breaker activ, ascundem modalul de recuperare"
        );
        setIsVisible(false);
      }
    }
  ...
  const handleCleanupAndLogout = async () => {
    ...
    try {
      ...
      setMessage(
        "Datele au fost curățate cu succes. Veți fi redirecționat la pagina de autentificare..."
      );
      ...
      setMessage(
        "A apărut o eroare la curățarea datelor. Reîncărcați pagina și încercați din nou."
      );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
        <h2 className="text-xl font-bold text-red-600 mb-4">
          Problemă de autentificare detectată
        </h2>

        <p className="mb-4">
          Am detectat o problemă cu datele de autentificare. Acest lucru poate
          cauza erori repetate și probleme de performanță în aplicație.
        </p>

        <p className="mb-4">
          Pentru a rezolva problema, trebuie să curățăm datele salvate și să vă
          reconectați. Acest proces nu vă va afecta datele personale sau alte
          setări.
        </p>
        ...
        <div className="flex justify-end space-x-4 mt-6">
          ...
          >
            {isProcessing
              ? "Se procesează..."
              : "Curăță datele și reconectează-te"}
          </button>
        </div>
      </div>
    </div>
