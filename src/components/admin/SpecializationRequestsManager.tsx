...
const SpecializationRequestsManager: React.FC<{ adminId: string }> = ({ adminId }) => {
  const [requests, setRequests] = useState<SpecializationRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedRequest, setSelectedRequest] = useState<SpecializationRequest | null>(null);
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      setRequests((response.requests || []) as unknown as SpecializationRequest[]);
    } catch (error) {
        ...
        setRequests(prevRequests =>
          prevRequests.map(req =>
            req.id === selectedRequest.id
              ? { ...req, status, comments: adminComment, processedAt: new Date() }
              : req
          )
        );
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-semibold mb-4">Cereri de schimbare a specializării</h2>
                ...
                <tr key={request.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-500">
                      <div>
                        <span className="font-medium">Veche:</span> {request.oldSpecialization || "N/A"}
                      </div>
                      <div>
                        <span className="font-medium">Nouă:</span> {request.newSpecialization}
                      </div>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4 overflow-hidden">
            <div className="bg-gray-100 px-6 py-4 border-b flex justify-between items-center">
              <h3 className="text-lg font-medium text-gray-900">Detalii cerere</h3>
              <button
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Specialist</h4>
                  <p className="text-base font-medium text-gray-900">{selectedRequest.displayName}</p>
                  <p className="text-sm text-gray-600">{selectedRequest.userEmail}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Data cererii</h4>
                  <p className="text-sm text-gray-800">{formatDate(selectedRequest.submittedAt)}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Specializare actuală</h4>
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Specializare solicitată</h4>
              ...
              {selectedRequest.specializationDetails && (
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Detalii specializare</h4>
              ...
              <div className="mb-4">
                <h4 className="text-sm font-medium text-gray-500 mb-1">Motivul solicitării</h4>
                <p className="text-sm text-gray-800 bg-gray-50 p-3 rounded">
              ...
              {selectedRequest.status !== "pending" && (
                <div className="mb-4">
                  ...
                  >
                    {selectedRequest.status === "approved" ? "Aprobat" : "Respins"}
                    {selectedRequest.processedAt &&
                      ` la ${formatDate(selectedRequest.processedAt as Date)}`}
                  </p>
                  {selectedRequest.comments && (
                    <div className="mt-2">
                      <h4 className="text-sm font-medium text-gray-500 mb-1">Comentarii admin</h4>
                      <p className="text-sm text-gray-800 bg-gray-50 p-3 rounded">
                        {selectedRequest.comments}
                      </p>
                    </div>
              ...
              {selectedRequest.status === "pending" && (
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Comentarii (opțional)</h4>
                  <textarea
                    id="adminComment"
                    value={adminComment}
