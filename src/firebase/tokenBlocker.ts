...
export const initTokenBlocker = (): void => {
  ...
  blockerLogger.info(
    "Token Blocker inițializat - monitorizăm cererile problematice de token"
  );
    ...
    if (url.includes("securetoken.googleapis.com")) {
      ...
      if (isBlocked()) {
        const remainingBlockTime = Math.ceil(
          (state.blockDuration - (Date.now() - (state.blockStartTime || 0))) /
            1000
        );

        blockerLogger.warn(
          `Cerere blocată către ${url} - Rămân ${remainingBlockTime} secunde de blocare`
        );

        // În loc să facem cererea, aruncăm o eroare controlată
        throw new Error(
          `BLOCKED_TOKEN_REQUEST: Cerere blocată pentru a preveni problemele de autentificare. Reîncărcați pagina peste ${remainingBlockTime} secunde.`
        );
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 400 || response.status === 429) {
            blockTokenRequests(
              "Eroare HTTP critică detectată: " + response.status
            );
          }
        }
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    if (urlStr.includes("securetoken.googleapis.com") && isBlocked()) {
      ...
      blockerLogger.warn(
        `XHR cerere blocată către ${urlStr} - Rămân ${remainingBlockTime} secunde de blocare`
      );
  ...
  console.error = (...args: unknown[]): void => {
    ...
    if (
      ...
      if (
        ...
        blockTokenRequests(
          "Eroare critică detectată în consolă: " + errorMsg.substring(0, 50)
        );
      }
    }
  };
...
export const isBlocked = (): boolean => {
  ...
  if (
    ...
    now - state.blockStartTime >= state.blockDuration
  ) {
    // Perioada de blocare a expirat, resetăm blocarea
    state.isTokenFetchBlocked = false;
    blockerLogger.info(
      "Perioada de blocare a expirat - reactivăm cererile de token"
    );
    return false;
  }
...
const handleTokenError = (errorMsg: string): void => {
  ...
  if (
    ...
    blockerLogger.error(
      `Eroare critică de token detectată: ${errorMsg.substring(0, 100)}...`
    );
  }
  ...
  if (
    state.errorCount >= ERROR_THRESHOLD &&
    state.lastErrorTime &&
    now - state.lastErrorTime < ERROR_WINDOW
  ) {
    blockTokenRequests(
      `Prag de erori atins (${state.errorCount}/${ERROR_THRESHOLD} într-un interval de ${ERROR_WINDOW / 1000}s)`
    );
  }
};
...
export const blockTokenRequests = (reason: string): void => {
  if (state.isTokenFetchBlocked) {
    ...
    blockerLogger.warn(
      `Blocarea cererilor de token a fost extinsă la ${state.blockDuration / 60000} minute: ${reason}`
    );
  ...
  blockerLogger.warn(
    `Cererile de token au fost blocate pentru ${blockMinutes} minute: ${reason}`
  );
  ...
  setTimeout(() => {
    if (state.criticalErrorDetected) {
      blockerLogger.info(
        "Perioada de blocare s-a încheiat, dar au fost detectate erori critice - reîncărcăm pagina"
      );
      ...
      reloadMsg.textContent =
        "Reîmprospătăm pagina pentru a rezolva problemele de autentificare...";
...
const showBlockNotification = (blockMinutes: number): void => {
  ...
  document
    .getElementById("dismiss-notification-btn")
