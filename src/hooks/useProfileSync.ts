...
import syncUserProfile, { ProfileSyncService } from "../services/ProfileSyncService";
 ...
 */
export const useProfileSync = (options = { syncInterval: 120000 }) => { // Crescut la 2 minute (era 60 secunde)
  ...
  const synchronizeProfile = useCallback(async (force = false) => {
    ...
    if (!force && timeSinceLastSync < MIN_SYNC_INTERVAL) {
      hookLogger.debug(`Sincronizare ignorată: prea curând de la ultima încercare (${timeSinceLastSync}ms)`);
      return false;
    }
    ...
    try {
      ...
      if (isQuotaExceeded) {
        ...
        hookLogger.warn("Cota Firebase a fost depășită, vom limita sincronizările");
      }
  ...
  const checkAndSync = useCallback(async () => {
    ...
    if (timeSinceLastSync < MIN_SYNC_INTERVAL) {
      hookLogger.debug(`Verificare ignorată: prea curând de la ultima încercare (${timeSinceLastSync}ms)`);
      return false;
    }
  ...
  useEffect(() => {
    if (user && !quotaExceeded) {
      const importantRoutes = ["/admin", "/user-home", "/profile", "/dashboard", "/settings"];
      const shouldSync = importantRoutes.some(route => location.pathname.includes(route));
      
      if (shouldSync) {
        hookLogger.debug("Ruta s-a schimbat, verificăm profilul:", location.pathname);
        synchronizeProfile();
      } else {
        hookLogger.debug("Ruta s-a schimbat, dar nu e necesară sincronizarea:", location.pathname);
      }
    }
  ...
  useEffect(() => {
    ...
    const adaptiveInterval = syncErrors > 0 
      ? Math.min(options.syncInterval * Math.pow(2, syncErrors), 900000) // Maxim 15 minute (era 5 minute)
      : options.syncInterval;
    
    hookLogger.debug(`Setăm interval de sincronizare adaptiv: ${adaptiveInterval}ms`);
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((authUser) => {
      if (authUser) {
        hookLogger.debug("Detectată schimbare în starea de autentificare, sincronizăm profilul");
