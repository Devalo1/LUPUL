...
const AdminAnalytics: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "reading" | "engagement" | "reports" | "ai"
  >("overview");
  ...
  const generateMonthlyReport = async () => {
    try {
      ...
      generateCSVReport(
        monthlyData,
        `raport-lunar-${new Date().toISOString().slice(0, 7)}.csv`
      );
    } catch (error) {
  ...
  const generateBehaviorReport = async () => {
    try {
      ...
      generateCSVReport(
        behaviorData,
        `raport-comportament-${new Date().toISOString().slice(0, 10)}.csv`
      );
    } catch (error) {
  ...
  const generateArticlePerformanceReport = async () => {
    try {
      ...
      generateCSVReport(
        articleData,
        `raport-performanta-articole-${new Date().toISOString().slice(0, 10)}.csv`
      );
    } catch (error) {
      console.error(
        "Eroare la generarea raportului de performanță articole:",
        error
      );
    }
  };
  ...
  const generateConversionReport = async () => {
    try {
      ...
      generateCSVReport(
        conversionData,
        `raport-conversii-${new Date().toISOString().slice(0, 10)}.csv`
      );
    } catch (error) {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const bounceRate =
        totalUsers > 0
          ? Math.max(0, 100 - (returnVisitors / totalUsers) * 100)
          : 0;

      // Calculează timpul mediu pe pagină - estimat pe baza angajamentului
      const averageEngagement =
        articles.length > 0
          ? articles.reduce((sum, article) => sum + article.engagement, 0) /
            articles.length
          : 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-sm text-blue-600 mt-1">
            Descarcă raportul complet pentru ultima lună
          </p>
        </button>
        ...
        >
          ...
          <p className="text-sm text-green-600 mt-1">
            Raport despre comportamentul utilizatorilor
          </p>
        </button>
        ...
        >
          <h4 className="font-medium text-purple-800">
            Performanța Articolelor
          </h4>
          <p className="text-sm text-purple-600 mt-1">
            Statistici detaliate pentru fiecare articol
          </p>
        </button>
        ...
        >
          ...
          <p className="text-sm text-orange-600 mt-1">
            Analiza conversiilor și obiectivelor
          </p>
        </button>
      </div>
    </div>
