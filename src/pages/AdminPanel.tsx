...
const AdminPanel: React.FC = () => {
  ...
  const [specialistActionInProgress, setSpecialistActionInProgress] = useState<string | null>(null);
  const [messageAlert, setMessageAlert] = useState<{type: "success" | "error", message: string} | null>(null);
  const [roleChangeRequests, setRoleChangeRequests] = useState<Array<{
    id: string;
    userId: string;
    userEmail: string;
    userName: string;
  ...
  const fetchRoleChangeRequests = useCallback(async () => {
    try {
      ...
      for (const requestDoc of querySnapshot.docs) {
        ...
        if (userDoc.exists()) {
          requestsList.push({
            ...
            createdAt: requestData.createdAt?.toDate ? requestData.createdAt.toDate() : requestData.createdAt,
      ...
      console.error("Eroare la încărcarea cererilor de schimbare a rolului:", error);
    } finally {
  ...
  const handleMakeSpecialist = async (userId: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        setMessageAlert({
          type: "error",
          message: "Nu s-a putut seta utilizatorul ca specialist. Încercați din nou."
        });
      }
    } catch (error) {
      ...
      setMessageAlert({
        type: "error",
        message: "Eroare la setarea rolului de specialist. Verificați consola pentru detalii."
      });
    } finally {
  ...
  const handleRemoveSpecialist = async (userId: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        setMessageAlert({
          type: "error",
          message: "Nu s-a putut elimina rolul de specialist. Încercați din nou."
        });
      }
    } catch (error) {
      ...
      setMessageAlert({
        type: "error",
        message: "Eroare la eliminarea rolului de specialist. Verificați consola pentru detalii."
      });
    } finally {
      setSpecialistActionInProgress(null);
  ...
  const handleApproveRoleChange = async (requestId: string) => {
    ...
    try {
      ...
      await processRoleChangeRequest(requestId, "approved", user?.email || undefined);
      
      setMessageAlert({
        type: "success",
        message: "Cererea a fost aprobată și utilizatorul a fost setat ca specialist!"
      });
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mt-6">
          ...
          {messageAlert && (
            <div className={`mb-6 p-4 rounded-md ${
              messageAlert.type === "success" ? "bg-green-50 text-green-800 border border-green-200" : 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-5 hover:shadow-md transition-shadow">
              ...
              <div className="flex flex-col space-y-2">
                ...
                <a href="#specialist-management" className="text-purple-600 hover:underline text-sm">👨‍⚕️ Gestionare specialiști</a>
              </div>
            </div>
          ...
          <div id="specialist-management" className="bg-white rounded-lg shadow-md p-6 mt-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Gestionare Specialiști</h2>
            <p className="text-gray-600 mb-6">
              Alocați sau revocați rolul de specialist pentru utilizatori. Specialiștii pot vedea și gestiona programările atribuite lor și pot trimite mesaje către clienți.
            </p>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          {userItem.role !== UserRole.SPECIALIST ? (
                            <button
                              ...
                              disabled={specialistActionInProgress === userItem.id}
                              className="text-green-600 hover:text-green-900 mr-4 disabled:opacity-50"
                            >
                              {specialistActionInProgress === userItem.id ? "Se procesează..." : "Setează ca specialist"}
                            </button>
                          ) : (
                            <button
                              onClick={() => handleRemoveSpecialist(userItem.id)}
                              disabled={specialistActionInProgress === userItem.id}
                              className="text-red-600 hover:text-red-900 mr-4 disabled:opacity-50"
                            >
                              {specialistActionInProgress === userItem.id ? "Se procesează..." : "Elimină rol specialist"}
                            </button>
                          )}
                        </td>
                      </tr>
          ...
          <div id="role-change-requests" className="bg-white rounded-lg shadow-md p-6 mt-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Cereri de schimbare a rolului</h2>
            <p className="text-gray-600 mb-6">
              Gestionați cererile utilizatorilor care doresc să devină specialiști pe platformă.
            </p>
            ...
            {loadingRoleRequests ? (
              ...
              <div className="bg-gray-50 p-6 text-center rounded-md">
                <p className="text-gray-600">Nu există cereri de schimbare a rolului în așteptare.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {roleChangeRequests
                      ...
                      <tr key={request.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          ...
                          >
                            {processingRequestId === request.id ? "Se procesează..." : "Respinge"}
                          </button>
                        </td>
                      </tr>
                    ))}
          ...
          <div id="dashboard-specialists" className="bg-white rounded-lg shadow-md p-6 mt-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Specialiști activi</h2>
            <p className="text-gray-600 mb-6">
              Aceasta este lista specialiștilor activi pe platformă și specializările lor.
            </p>
            ...
            {loadingUsers ? (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {users
                      .filter(userItem => userItem.role === UserRole.SPECIALIST)
                      .map((userItem) => (
                      <tr key={userItem.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                    ...
                    {users.filter(user => user.role === UserRole.SPECIALIST).length === 0 && (
                      <tr>
