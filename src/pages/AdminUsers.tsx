...
interface User {
  ...
  isAccountant?: boolean; // Adăugat pentru rolul de contabil
...
const AdminUsers: React.FC = () => {
  ...
  const [filter, setFilter] = useState("all"); // 'all', 'admin', 'regular', 'specialist', 'accountant'
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Get all specialists from the specialists collection as well
      const specialistsRef = collection(db, "specialists");
      const specialistsSnapshot = await getDocs(specialistsRef);
      const specialistsMap: { [key: string]: any } = {};
      
      specialistsSnapshot.docs.forEach(specialistDoc => {
        // Some specialist documents use userId field, others are directly keyed by user ID
        const userId = specialistDoc.data().userId || specialistDoc.id;
        specialistsMap[userId] = {
          ...specialistDoc.data(),
          isSpecialist: true
        };
      });

      // Get all accountants from the accountants collection as well
      const accountantsRef = collection(db, "accountants");
      const accountantsSnapshot = await getDocs(accountantsRef);
      const accountantsMap: { [key: string]: any } = {};
      
      accountantsSnapshot.docs.forEach(accountantDoc => {
        const userId = accountantDoc.data().userId || accountantDoc.id;
        accountantsMap[userId] = {
          ...accountantDoc.data(),
          isAccountant: true
        };
      });
      ...
      const usersList = await Promise.all(usersSnapshot.docs.map(async (userDoc) => {
        ...
        // Determine if user is a specialist - check multiple sources
        const isSpecialist = 
          // Check if user document has specialist role
          userData.role === "specialist" || 
          userData.isSpecialist === true || 
          // Check if user exists in specialists map
          specialistsMap[userDoc.id] !== undefined || 
          // Check roles map
          rolesMap[userDoc.id] === "specialist";

        // Determine if user is an accountant - check multiple sources
        const isAccountant = 
          userData.role === "accountant" || 
          userData.isAccountant === true || 
          accountantsMap[userDoc.id] !== undefined || 
          rolesMap[userDoc.id] === "accountant";
        
        // Determine final role with priority: admin > specialist > accountant > user
        let role = "user";
        if (isAdmin) {
          role = "admin";
  ...
  const makeAdmin = async (userId: string) => {
    try {
      ...
      setUsers(users.map(user => 
        user.id === userId ? { ...user, isAdmin: true, role: "admin", isSpecialist: false, isAccountant: false } : user
      ));
  ...
  const makeSpecialist = async (userId: string) => {
    try {
      ...
      // Actualizează lista locală
      setUsers(users.map(user => 
        user.id === userId ? { ...user, isSpecialist: true, role: "specialist", isAdmin: false, isAccountant: false } : user
      ));
  ...
  const makeAccountant = async (userId: string) => {
    try {
      // Adaugă utilizatorul ca contabil
      await makeUserAccountant(userId);
      
      // Actualizează lista locală
      setUsers(users.map(user => 
        user.id === userId ? { ...user, isAccountant: true, role: "accountant", isAdmin: false, isSpecialist: false } : user
      ));
      
      alert("Utilizatorul a fost setat ca Contabil.");
    } catch (error) {
      console.error("Error making user accountant:", error);
      alert("Eroare la adăugarea utilizatorului ca contabil. Vă rugăm încercați din nou.");
    }
  };
  
  const removeAccountant = async (userId: string) => {
    try {
      // Revocă rolul de contabil
      await removeAccountantRole(userId);
      
      // Actualizează lista locală
      setUsers(users.map(user => 
        user.id === userId ? { ...user, isAccountant: false, role: "user" } : user
      ));
      
      alert("Rolul de contabil a fost revocat pentru acest utilizator.");
    } catch (error) {
      console.error("Error removing accountant status:", error);
      alert("Eroare la revocarea rolului de contabil. Vă rugăm încercați din nou.");
    }
  };
  ...
  const filteredUsers = users
    .filter(user => {
      ...
      switch (filter) {
        ...
        case "regular":
          return matchesSearch && !user.isAdmin && !user.isSpecialist && !user.isAccountant;
        case "specialist":
        ...
        <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 gap-4 mb-6">
          ...
          <div className="w-full md:w-48">
            ...
            >
              ...
              <option value="accountant">Doar contabili</option>
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-4 px-4 whitespace-nowrap">
                        ...
                        {!user.isAdmin && !user.isSpecialist && !user.isAccountant ? (
                          ...
                          <button 
                            className="text-red-600 hover:text-red-900"
                            onClick={() => {
                              if(confirm("Sunteți sigur că doriți să revocați rolul de contabil pentru acest utilizator?")) {
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-full max-w-md mx-4 p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-gray-200">
                ...
                {!selectedUser.isAdmin && !selectedUser.isSpecialist && !selectedUser.isAccountant ? (
