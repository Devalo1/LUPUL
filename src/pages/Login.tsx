...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      loginLogger.info("Încercare de autentificare pentru:", email);
      await authService.login(email, password);
      
      // Redirect to the page they were trying to access
      loginLogger.info("Autentificare reușită, redirectionare către:", from);
      navigate(from, { replace: true });
    } catch (err: any) {
      loginLogger.error("Eroare la autentificare:", err);
      ...
      if (err.code === "auth/user-not-found" || err.code === "auth/wrong-password") {
        ...
        errorMessage = "Prea multe încercări eșuate. Încercați din nou mai târziu.";
      } else if (err.message) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      loginLogger.info("Încercare de autentificare cu Google");
      const result = await authService.loginWithGoogle(from);
      
      if (result.success && result.redirectPath) {
        loginLogger.info("Autentificare Google reușită, redirectionare către:", result.redirectPath);
