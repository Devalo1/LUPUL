...
const ProductDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setError("A apărut o eroare la încărcarea produsului. Vă rugăm să încercați din nou.");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="container mx-auto px-4 py-16">
        <div className="text-center">
          ...
          <button onClick={() => navigate("/magazin")} className="px-4 py-2 bg-blue-600 text-white rounded">
            Înapoi la produse
          </button>
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto px-4 py-16 bg-stone-50">
      ...
      {/* Back navigation */}
      <div className="mb-8">
        <Link to="/magazin" className="text-blue-600 hover:underline flex items-center">
              ...
              {relatedProducts.map(product => (
                <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden transform transition hover:scale-105">
                  ...
                  <div className="p-4">
                    ...
                    <Link to={`/product/${product.id}`} className="block mt-3 w-full py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors text-center">
                      Vezi detalii
                    </Link>
                  </div>
                </div>
