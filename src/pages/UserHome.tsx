...
import { useProfileSync as _useProfileSync } from "../hooks/useProfileSync";
...
const UserHome: React.FC = () => {
  ...
  // State-uri pentru filtrarea și afișarea articolelor
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedTag, setSelectedTag] = useState<string | null>(null);
  const [visibleArticles, setVisibleArticles] = useState<Article[]>([]);
  const [allTags, setAllTags] = useState<string[]>([]);

  // Reîmprospătează datele profilului la încărcarea paginii
  useEffect(() => {
    const refreshProfile = async () => {
      if (user && !loading) {
        try {
          // Utilizăm direct refreshUserData din destructurarea inițială
          await refreshUserData();
          console.log("Profil utilizator reîmprospătat în UserHome");
        } catch (error) {
