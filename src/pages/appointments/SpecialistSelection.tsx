...
import { FaUserMd, FaInfoCircle, FaExternalLinkAlt, FaEdit, FaUser } from "react-icons/fa";
...
// Helper function to generate initials from name - prefixed with underscore to avoid ESLint warnings
const _getSpecialistInitials = (name: string): string => {
  if (!name) return "?";
...
// Helper function to get consistent background color for a specialist - prefixed with underscore to avoid ESLint warnings
const _getSpecialistBgColor = (id: string): string => {
  const colors = [
...
const SpecialistSelection: React.FC = () => {
    ...
    const fetchSpecialists = async () => {
      try {
        ...
        try {
          ...
          console.log(`Specialists collection raw count: ${specialistsSnapshot.docs.length}`);
          
          if (!specialistsSnapshot.empty) {
            // Log complete data pentru debugging
            console.log("Raw specialists data:", specialistsSnapshot.docs.map(doc => {
              const data = doc.data();
              return {
                id: doc.id, 
            ...
            const specialistsFromCollection = specialistsSnapshot.docs
              .map(doc => {
                ...
                console.log(`Specialist [${doc.id}] processed fields:`, {
                  ...
                  imageUrl: data.photoURL || data.imageUrl || data.avatarURL || "No image",
                  role: data.role || data.specialization || data.serviceType || "No role",
                  isActive: data.isActive !== false // true dacă nu e explicit false
                });
                
                // Convertim și curățăm URL-ul imaginii dacă există
                let imageUrl = data.photoURL || data.imageUrl || data.avatarURL || "";
                let photoURL = data.photoURL || data.imageUrl || data.avatarURL || "";
                ...
                if (photoURL && typeof photoURL === "string") {
                  if (photoURL.includes("firebasestorage.googleapis.com") && !photoURL.includes("alt=media")) {
                ...
                return {
                  id: doc.id,
                  name: data.name || data.displayName || data.fullName || "Specialist",
                  role: data.role || data.specialization || data.serviceType || "Specialist",
                  imageUrl: imageUrl,
                  photoURL: photoURL, // Adăugăm explicit photoURL
                  description: data.description || data.bio || "Specialist în domeniul sănătății",
                  serviceType: data.serviceType || data.specializationCategory || "",
                  isActive: data.isActive !== false, // true dacă nu e explicit false
                  email: data.email || "",
                  phone: data.phone || "",
              ...
              .filter(s => {
                ...
                if (!isActive) console.log(`Excluding inactive specialist: ${s.id} - ${s.name}`);
                return isActive;
              });
            
            console.log(`Found ${specialistsFromCollection.length} active specialists in collection`);
        ...
        try {
          ...
          console.log(`Users collection raw count: ${usersSnapshot.docs.length}`);
          ...
          if (!usersSnapshot.empty) {
            ...
            const potentialSpecialists = usersSnapshot.docs.filter(doc => {
              ...
              if (isSpecialist) {
                console.log(`Found specialist user: ${doc.id}, Name: ${userData.displayName || userData.name || "No name"}`);
              }
              
              return isSpecialist && !specialistIds.has(doc.id) && userData.isActive !== false;
            });
            
            console.log(`Found ${potentialSpecialists.length} potential specialists in users collection`);
            ...
            const specialistsFromUsers = potentialSpecialists.map(doc => {
              ...
              console.log(`User specialist [${doc.id}] processed fields:`, {
                name: userData.displayName || userData.name || userData.email || "No name",
                imageUrl: userData.photoURL || userData.avatarURL || userData.imageUrl || "No image",
                role: userData.specialization || userData.serviceType || userData.role || "Specialist"
              });
              
              // Procesăm URL-ul imaginii similar cu codul anterior
              let imageUrl = userData.photoURL || userData.avatarURL || userData.imageUrl || "";
              let photoURL = userData.photoURL || userData.avatarURL || userData.imageUrl || "";
              
              if (imageUrl && typeof imageUrl === "string") {
                console.log(`Original imageUrl for user specialist ${doc.id}:`, imageUrl);
                ...
                console.log(`Processed imageUrl for user specialist ${doc.id}:`, imageUrl);
              }
              ...
              return {
                ...
                imageUrl: imageUrl,
                photoURL: photoURL,
                description: userData.bio || userData.description || "Specialist în domeniul sănătății și stării de bine.",
                serviceType: userData.serviceType || userData.specializationCategory || "",
                isActive: true,
            ...
            console.log(`Found ${specialistsFromUsers.length} specialists in users collection after processing`);
        ...
        console.log("Final specialists list:", specialistsList.map(s => ({
          id: s.id,
          name: s.name,
          hasImage: !!s.imageUrl,
          imageUrl: s.imageUrl,
        ...
        if (specialistsList.length > 0) {
          ...
          console.log(`Successfully set ${specialistsList.length} specialists in state`);
        } else {
  ...
  const handleContinue = () => {
    if (selectedSpecialist) {
      ...
      navigate("/appointments/service", { state: { appointmentData: updatedData } });
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
      <div className="container mx-auto px-4 py-12">
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          ...
          <p className="text-gray-600 mb-8 border-b pb-4">Pasul 1: Alegerea specialistului</p>
              ...
              <div className="flex">
                <div className="flex-shrink-0">
                  <svg className="h-5 w-5 text-red-500" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 101.414 1.414L10 11.414l1.293-1.293a1 1 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                  </svg>
                </div>
                <div className="ml-3">
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={`step-${i}`} className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    i === 1 ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-500"
                  }`}>
                    {i}
                  </div>
                      ...
                      {specialists.map((specialist, index) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              console.log("Selected specialist:", specialist?.id);
                          ...
                          >
                            <div className="flex flex-col md:flex-row items-center mb-3">
                              <div className="w-24 h-24 rounded-full overflow-hidden flex-shrink-0 mb-3 md:mb-0 transition transform hover:scale-105 shadow">
                                <SpecialistAvatar
                                  photoURL={specialist.photoURL || specialist.imageUrl}
                                  name={specialistName}
                              ...
                              <div className="md:ml-4 text-center md:text-left flex-grow">
                                ...
                                <p className="text-gray-600">{specialist?.role || "Specialist"}</p>
                            ...
                            <p className="text-sm text-gray-700 mt-2 border-t pt-2">
                              {specialist?.description || "Specialist în domeniul sănătății și stării de bine."}
                            </p>
                            ...
                            <div className="mt-4 flex justify-between items-center">
                              ...
                              <button
                                onClick={(e) => handleViewProfile(specialist.id, e)}
                                className="text-blue-600 hover:text-blue-800 text-sm flex items-center ml-auto"
                              >
                                Vezi profilul complet <FaExternalLinkAlt className="ml-1" size={12} />
                              </button>
                            </div>
                          </div>
                        );
