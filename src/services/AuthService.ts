...
export class AuthService {
  ...
  async loginWithGoogle(redirectPath?: string): Promise<GoogleAuthResult> {
    try {
      ...
      // Configure cross-origin headers to prevent COOP errors
      this.configureCrossOriginHeaders();
      ...
      // Setăm parametri custom pentru a evita problemele COOP
      provider.setCustomParameters({
        prompt: "select_account",
        // Parametrii pentru a reduce problemele Cross-Origin-Opener-Policy
      ...
      try {
        ...
        authLogger.debug("Deschidere popup autentificare Google...");
        ...
        // Handle popup errors specifically for COOP issues
        ...
        if (errorCode === "auth/popup-closed-by-user" || 
          ...
          authLogger.warn("Eroare COOP detectată, încercăm cu redirect:", err.message);
          
          // Salvăm redirectPath pentru a fi folosit după redirecționare
          localStorage.setItem("authRedirectPath", redirectPath || "/dashboard");
          
          // Folosim redirect ca fallback
          const redirectProvider = new GoogleAuthProvider();
          redirectProvider.addScope("profile");
          redirectProvider.addScope("email");
          
          // Redirecționăm utilizatorul - acest proces va părăsi pagina curentă
          await signInWithRedirect(auth, redirectProvider);
          
          // Acest cod nu va fi executat până când utilizatorul nu se întoarce
          return {
            success: false,
            error: "Autentificare în curs prin redirecționare"
          };
        }
  ...
  /**
   * Configurează header-urile Cross-Origin pentru a permite popup-urile Firebase Auth
   */
  private configureCrossOriginHeaders(): void {
    if (typeof document !== "undefined") {
      // Verificăm dacă meta tag-ul există deja
      let metaTag = document.querySelector("meta[http-equiv=\"Cross-Origin-Opener-Policy\"]");
      
      if (!metaTag) {
        metaTag = document.createElement("meta");
        metaTag.setAttribute("http-equiv", "Cross-Origin-Opener-Policy");
        metaTag.setAttribute("content", "same-origin-allow-popups");
        document.head.appendChild(metaTag);
        authLogger.debug("Header Cross-Origin-Opener-Policy configurat pentru autentificare");
      }
    }
  }
