...
export const syncUserProfile = async (force = false): Promise<boolean> => {
  ...
  if (isSyncingSuspended && !force) {
    profileLogger.warn(
      "Sincronizarea profilului este suspendată temporar din cauza erorilor anterioare"
    );
    return false;
  }
  ...
  if (!force && now - lastSyncTimestamp < 1000) {
    profileLogger.warn(
      "Prea multe cereri de sincronizare într-un interval scurt, ignorăm"
    );
    return false;
  }
  ...
  if (!tokenValid) {
    ...
    if (!force) {
      profileLogger.error(
        "Token-ul este invalid. Suspendăm sincronizarea profilului temporar"
      );
      ...
      setTimeout(
        () => {
          profileLogger.info(
            "Reactivăm sincronizarea profilului după perioada de așteptare"
          );
      ...
      if (timeSinceLastHeal < 30000 && tokenHealingAttempts > 0) {
        profileLogger.warn(
          `Încercare de reparare token ignorată - a trecut prea puțin timp de la ultima încercare (${Math.floor(timeSinceLastHeal / 1000)}s)`
        );
      } else if (tokenHealingAttempts < MAX_TOKEN_HEALING_ATTEMPTS) {
        // Îmbunătățim logarea pentru acest caz
        profileLogger.warn(
          "Token-ul este invalid, dar continuăm sincronizarea forțată la cererea explicită a aplicației"
        );
        ...
        if (currentUser) {
          ...
          profileLogger.info(
            `Încercăm să reparăm token-ul înainte de sincronizarea forțată (încercarea ${tokenHealingAttempts}/${MAX_TOKEN_HEALING_ATTEMPTS})`
          );
          try {
            ...
            if (healed) {
              ...
              profileLogger.warn(
                "Nu s-a putut repara token-ul, continuăm oricum cu sincronizarea forțată"
              );
              ...
              if (tokenHealingAttempts >= MAX_TOKEN_HEALING_ATTEMPTS) {
                profileLogger.error(
                  `Număr maxim de încercări de reparare token atins (${MAX_TOKEN_HEALING_ATTEMPTS}), suspendăm repararea pentru 10 minute`
                );
                ...
                setTimeout(
                  () => {
                    ...
                    isSyncingSuspended = false;
                    profileLogger.info(
                      "Resetăm contorul de încercări de reparare token și reactivăm sincronizarea"
                    );
                  },
                  10 * 60 * 1000
                );
            ...
            profileLogger.error(
              "Eroare la încercarea de reparare a token-ului:",
              healError
            );
          }
        }
        ...
        profileLogger.error(
          `Prea multe încercări consecutive de reparare a token-ului (${tokenHealingAttempts}), nu mai încercăm`
        );
        return false;
      }
    }
  ...
  if (!currentUser) {
    profileLogger.warn(
      "Încercare de sincronizare a profilului fără utilizator autentificat"
    );
    return false;
  }
  ...
  try {
    ...
    profileLogger.info(
      "Începe sincronizarea profilului pentru utilizatorul",
      currentUser.uid
    );
    ...
    if (!userDoc.exists()) {
      ...
      profileLogger.warn(
        "Utilizatorul nu există în Firestore, se creează un nou document"
      );
    ...
    if (
      ...
      profileLogger.info(
        "Detectată discrepanță între Firebase Auth și Firestore"
      );
      ...
      if (
        ...
        try {
          ...
          const photoURL =
            userData.photoURL &&
            userData.photoURL.length > MAX_FIREBASE_AUTH_URL_LENGTH
              ? generateAvatarUrl(
                  userData.displayName || currentUser.displayName
                )
              : userData.photoURL;
          ...
          if (
            String(updateError).includes("auth/invalid-profile-attribute") &&
            String(updateError).includes("Photo URL too long")
          ) {
            profileLogger.warn(
              "URL-ul fotografiei este prea lung pentru Firebase Auth, folosim avatar generat"
            );
            ...
            try {
              ...
              profileLogger.error(
                "Eroare la actualizarea profilului cu avatar generat:",
                fallbackError
              );
            }
          } else {
      ...
      if (
        ...
        TokenManager.canRequestToken()
      ) {
        profileLogger.info(
          `Token age is ${Math.round(tokenAge)} minutes, refreshing token`
        );
        // Folosim managerul de tokenuri în loc să apelăm direct getIdToken
        await TokenManager.getIdTokenSafely(currentUser, true);
      } else {
        profileLogger.debug(
          `Token refresh skipped: age=${Math.round(tokenAge)} min, valid=${TokenManager.isTokenValid()}, canRequest=${TokenManager.canRequestToken()}`
        );
      }
    ...
    if (
      ...
      profileLogger.error(
        "Eroare de autentificare la sincronizarea profilului:",
        error
      );
      ...
      if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
        profileLogger.error(
          "Suspendăm sincronizarea profilului din cauza erorilor repetate de autentificare"
        );
        isSyncingSuspended = true;
        ...
        if (
          ...
          profileLogger.error(
            "Eroare de token detectată, resetăm backoff-ul token-ului după 10 minute"
          );
...
export const ProfileSyncService = {
  ...
  async checkForChanges(): Promise<boolean> {
    ...
    if (isSyncingSuspended) {
      profileLogger.debug(
        "Verificare ignorată - sincronizarea este suspendată"
      );
      return false;
    }
  ...
  async forceSynchronization(): Promise<boolean> {
    ...
    if (tokenHealth.circuitBreakerActive) {
      profileLogger.warn(
        "Nu se poate forța sincronizarea - circuit breaker token activ"
      );
      return false;
    }
    ...
    if (!TokenManager.canRequestToken()) {
      profileLogger.warn(
        "Nu se poate forța sincronizarea - token-ul este în perioada de backoff"
      );
      return false;
    }
    ...
    if (!TokenManager.isTokenValid() && auth.currentUser) {
      profileLogger.warn(
        "Token invalid detectat, încercăm repararea înainte de sincronizarea forțată"
      );
      try {
        const healed = await TokenManager.healToken(auth.currentUser);
        if (healed) {
          profileLogger.info(
            "Token reparat cu succes, continuăm cu sincronizarea"
          );
        } else {
          profileLogger.warn(
            "Repararea token-ului a eșuat, dar continuăm cu sincronizarea forțată"
          );
        }
      } catch (error) {
  ...
  async synchronizeIfNeeded(): Promise<boolean> {
    ...
    if (
      (window as any).__lastSyncDebounce &&
    ...
    if (isCircuitBreakerActive) {
      profileLogger.debug(
        "Sincronizare ignorată - circuit breaker token activ"
      );
      return false;
    }
    ...
    if (needsSync) {
      profileLogger.info(
        "Sincronizarea necesară - s-au detectat modificări în profil"
      );
  ...
  async forceTokenReset(): Promise<boolean> {
    profileLogger.warn(
      "Se forțează resetarea completă a token-urilor și a stării de sincronizare"
    );
  ...
  getSyncStatus(): Record<string, any> {
    return {
      ...
      lastChangeCheckTimestamp:
        lastChangeCheckTimestamp > 0
          ? new Date(lastChangeCheckTimestamp).toISOString()
          : null,
      lastTokenHealAttempt:
        lastTokenHealAttempt > 0
          ? new Date(lastTokenHealAttempt).toISOString()
          : null,
