...
import { ref, uploadBytes, getDownloadURL, deleteObject as _deleteObject } from "firebase/storage";
...
class SimplePhotoUploader {
  ...
  static async uploadPhoto(file: File, userId: string): Promise<string> {
    try {
      ...
      // Încărcăm fișierul
      const _uploadResult = await uploadBytes(storageRef, file, {
      ...
      try {
        ...
        localStorage.setItem(`user_${userId}_photoURL_timestamp`, timestamp.toString());
      } catch (storageError) {
   ...
   */
  private static async updateAllUserReferences(userId: string, photoURL: string): Promise<void> {
    try {
      ...
      console.log("Toate referințele utilizatorului au fost actualizate cu noua imagine");
    } catch (error) {
      console.error("Eroare la actualizarea referințelor utilizatorului:", error);
    }
  }
  ...
  static async getProfilePhotoURL(userId: string): Promise<string | null> {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        const photoURL = userData.photoURL || userData.imageUrl || userData.avatarURL || userData.profileImage;
      ...
      if (specialistDoc.exists()) {
        ...
        const photoURL = specialistData.photoURL || specialistData.imageUrl || specialistData.avatarURL || specialistData.profileImage;
      ...
      console.error("Eroare la obținerea URL-ului fotografiei din Firebase:", e);
    }
