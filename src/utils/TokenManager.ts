...
class TokenManager {
  ...
  private static isCircuitBreakerActive(): boolean {
    ...
    const minutesRemaining = Math.ceil(
      (this.CIRCUIT_BREAKER_TIMEOUT - timeElapsed) / 60000
    );
    tokenLogger.debug(
      `Circuit breaker activ, ${minutesRemaining} minute rămase până la reactivare`
    );
  ...
  static canRequestToken(): boolean {
    ...
    if (this.tokenState.isInBackoff) {
      const backoffEndTime =
        this.tokenState.lastTokenRequestTime + this.tokenState.backoffInterval;
      const canRequest = now >= backoffEndTime;

      if (canRequest) {
        tokenLogger.info(
          "Perioada de backoff s-a încheiat, se pot face cereri noi de token"
        );
        this.tokenState.isInBackoff = false;
      } else {
        const remainingTime = Math.ceil((backoffEndTime - now) / 1000);
        tokenLogger.debug(
          `Încă în perioada de backoff: ${remainingTime} secunde rămase`
        );
      }
  ...
  static isTokenValid(): boolean {
    ...
    if (
      this.tokenState.isInBackoff &&
      this.tokenState.consecutiveFailures > 2
    ) {
      tokenLogger.debug(
        "Token considerat invalid din cauza backoff-ului și eșecurilor consecutive"
      );
      return false;
    }
    ...
    if (
      this.tokenState.lastTokenError &&
      ...
      tokenLogger.debug(
        `Token considerat invalid din cauza erorii anterioare: ${this.tokenState.lastTokenError}`
      );
      return false;
    }
  ...
  static async getIdTokenSafely(
    ...
    if (!this.canRequestToken()) {
      const backoffSeconds = Math.ceil(this.tokenState.backoffInterval / 1000);
      tokenLogger.warn(
        `Cerere de token ignorată - în backoff pentru încă ${backoffSeconds} secunde`
      );
      return null;
    }
    ...
    try {
      ...
      if (
        ...
        if (this.tokenState.consecutiveFailures >= 3) {
          TokenBlocker.blockTokenRequests(
            `Erori critice repetate de token: ${this.tokenState.consecutiveFailures}`
          );
        }
      ...
      if (
        this.tokenState.failureCount >= this.MAX_FAILURES_BEFORE_CIRCUIT_BREAK
      ) {
        tokenLogger.error(
          `Prea multe eșecuri globale (${this.tokenState.failureCount}), activăm circuit breaker-ul`
        );
        ...
        TokenBlocker.blockTokenRequests(
          "Circuit breaker activat după prea multe eșecuri"
        );
      }
  ...
  static async clearTokensAndLogout(): Promise<boolean> {
    ...
    if (now - this.tokenState.lastTokenRequestTime < 5000) {
      tokenLogger.warn(
        "Prea multe încercări de curățare într-un interval scurt, se ignoră"
      );
      return false;
    }

    tokenLogger.warn(
      "Se curăță complet toate token-urile și datele de autentificare"
    );
      ...
      try {
        if (auth.currentUser) {
          ...
          successMessage.textContent =
            "Datele de autentificare au fost curățate. Reîmprospătați pagina sau reconectați-vă.";
          ...
          tokenLogger.warn(
            "Am activat circuit breaker-ul pentru a preveni reluarea imediată a încercărilor"
          );

          // Activăm și blocarea globală
          TokenBlocker.blockTokenRequests(
            "Deconectare forțată pentru curățarea token-urilor"
          );
        }
      } catch (signOutError) {
        tokenLogger.error(
          "Eroare la deconectarea utilizatorului:",
          signOutError
        );
      }
  ...
  static async healToken(user: User): Promise<boolean> {
    ...
    if (now - this.tokenState.lastTokenRequestTime < 2000) {
      tokenLogger.warn(
        "Prea multe încercări de reparare într-un interval scurt, se ignoră"
      );
      return false;
    }
    ...
    try {
      ...
      const timeoutPromise = new Promise<null>((_, reject) =>
        setTimeout(
          () => reject(new Error("Timeout la obținerea token-ului")),
          5000
        )
      );
  ...
  static forcePageRefresh(): void {
    tokenLogger.warn(
      "Forțăm reîmprospătarea paginii pentru a rezolva problemele de autentificare"
    );
    ...
    message.innerHTML =
      "Reîmprospătăm pagina pentru a rezolva problemele de autentificare...<br>Vă rugăm să așteptați.";
