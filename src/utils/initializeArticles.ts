...
export const initializeArticles = async (): Promise<boolean> => {
  try {
    console.log("Checking if articles collection exists and has data...");
...
Facultatea de Psihologie`,
      ...
      try {
        ...
        for (const article of sampleArticles) {
          ...
          console.log(`Added sample article with ID: ${docRef.id}`);
        }
      ...
      console.log(`Articles collection already has ${snapshot.docs.length} documents.`);
      return false;
    }
  } catch (error) {
