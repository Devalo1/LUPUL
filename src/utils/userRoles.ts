...
export const isUserAdmin = async (userId: string): Promise<boolean> => {
  try {
    ...
    if (currentUser && currentUser.uid === userId) {
      try {
        // Use the TokenRefreshManager instead of directly calling getIdTokenResult
        const { TokenRefreshManager } = await import("./tokenRefreshManager");
        
        try {
          // Get a cached token or a properly rate-limited fresh token
          const token = await TokenRefreshManager.getIdToken(currentUser, false);
          
          // Parse the token manually rather than using getIdTokenResult
          const parts = token.split(".");
          if (parts.length === 3) {
            const payload = JSON.parse(atob(parts[1]));
            if (payload.claims?.role === "admin" || payload.claims?.admin === true) {
              console.log("User is an admin based on token claims");
              return true;
            }
          }
        } catch (tokenError) {
        ...
        console.error("Error importing TokenRefreshManager:", importError);
        // Fallback to traditional method
      }
      
      // Main admin check by email
    ...
    if (currentUser && currentUser.uid === userId) {
      try {
        ...
        try {
          ...
          if (parts.length === 3) {
            const payload = JSON.parse(atob(parts[1]));
            if (payload.claims?.role === "specialist") {
        ...
        console.error("Error importing TokenRefreshManager:", importError);
        // Fallback to traditional method if TokenRefreshManager can't be loaded
        try {
...
/**
 * Verifică dacă un utilizator are rolul de contabil
 * @param userId - ID-ul utilizatorului de verificat
 * @returns Promise<boolean> - True dacă utilizatorul este contabil, false în caz contrar
 */
export const isUserAccountant = async (userId: string): Promise<boolean> => {
  try {
    // Add logging for debugging
    console.log(`Checking if user ${userId} is an accountant`);
    ...
    if (currentUser && currentUser.uid === userId) {
      try {
        // Use the TokenRefreshManager instead of directly calling getIdTokenResult
        // This avoids quota exceeded errors by using caching and rate limiting
        const { TokenRefreshManager } = await import("./tokenRefreshManager");
        ...
        try {
          ...
          if (parts.length === 3) {
            const payload = JSON.parse(atob(parts[1]));
            if (payload.claims?.role === "accountant") {
              console.log("User is an accountant based on token claims");
              return true;
            }
          }
        } catch (tokenError) {
        ...
        // Fallback to traditional method if TokenRefreshManager can't be loaded
        try {
          const idTokenResult = await getIdTokenResult(currentUser, false);
          if (idTokenResult.claims.role === "accountant") {
            console.log("User is an accountant based on token claims");
            return true;
          }
        } catch (tokenError) {
    ...
    // Verificăm documentul utilizatorului
    const userRef = doc(firestore, "users", userId);
    ...
    if (userSnapshot.exists()) {
      ...
      if (userData.role === roleToString(UserRole.ACCOUNTANT) || userData.isAccountant === true) {
        console.log("User is an accountant based on user document fields");
        return true;
      }
    }
    
    // Verificăm și colecția de contabili dacă există
    const accountantsRef = collection(firestore, "accountants");
    const accountantSnapshot = await getDoc(doc(accountantsRef, userId));
    
    if (accountantSnapshot.exists()) {
      console.log("User is an accountant (direct document in accountants collection)");
      return true;
    }
    
    // Check by email in accountants collection as fallback
    if (currentUser && currentUser.email) {
      const userEmail = currentUser.email.toLowerCase();
      const accountantsByEmailQuery = query(
        accountantsRef,
        where("email", "==", userEmail)
      );
      
      try {
        const accountantsByEmailSnapshot = await getDocs(accountantsByEmailQuery);
        if (!accountantsByEmailSnapshot.empty) {
          console.log("User is an accountant based on matching email in accountants collection");
          return true;
        }
      } catch (queryError) {
        console.error("Error querying accountants by email:", queryError);
      }
    }
    
    console.log("User is not an accountant");
    return false;
  } catch (error) {
    console.error("Error checking if user is accountant:", error);
    return false;
  }
};
...
/**
 * Acordă rolul de contabil unui utilizator
 * @param userId - ID-ul utilizatorului căruia îi acordăm rolul de contabil
 */
export const makeUserAccountant = async (userId: string): Promise<void> => {
  try {
    ...
    // Actualizăm documentul utilizatorului cu rolul de contabil
    await updateDoc(userRef, {
      role: "accountant",
      isAccountant: true,
      updatedAt: Timestamp.now()
    ...
    // Adăugăm o intrare în colecția contabili
    await setDoc(doc(db, "accountants", userId), {
      userId,
      fullName: userData.displayName || userData.email || "Contabil",
      email: userData.email || "",
      phone: userData.phoneNumber || userData.phone || "",
      isActive: true,
      createdAt: Timestamp.now(),
    ...
    throw new Error("Nu s-a putut acorda rolul de contabil");
  }
};
...
/**
 * Revocă rolul de contabil al unui utilizator
 * @param userId - ID-ul utilizatorului de la care se revocă rolul de contabil
 */
export const removeAccountantRole = async (userId: string): Promise<void> => {
  try {
    // Actualizăm documentul utilizatorului
    const userRef = doc(db, "users", userId);
    await updateDoc(userRef, {
    ...
    // Actualizăm colecția de contabili
    const accountantRef = doc(db, "accountants", userId);
    const accountantDoc = await getDoc(accountantRef);
    ...
    console.error("Error removing accountant role:", error);
    throw new Error("Nu s-a putut revoca rolul de contabil");
  }
};
