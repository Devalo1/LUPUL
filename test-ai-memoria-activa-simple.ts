// Test simplificat pentru funcționalitatea de memorie activă AI
// Simulează funcționalitatea fără dependențe Firebase
...
class MockPersonalizationService {
  ...
  analyzeConversation(conversation: Conversation): ConversationInsights {
  ...
  generatePersonalizedContext(profile: UserPersonalityProfile): string {
  ...
  const testConversations: Conversation[] = [
    {
      ...
      messages: [
        ...
        },
        {
          ...
          sender: "user",
          content:
            "Poti sa imi dai un exemplu simplu de componentă funcțională?",
          timestamp: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
        },
        {
          id: "msg4",
...
try {
  ...
  if (result.success) {
    ...
    process.stderr.write("❌ Testul a eșuat!\n");
    process.exit(1);
  }
} catch (error) {
  process.stderr.write(`❌ Eroare în test: ${error}\n`);
  process.exit(1);
}
