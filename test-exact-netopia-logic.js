...
function simulateExactNetopiaConfig() {
    console.log('🔧 Simulez EXACT getNetopiaConfig() din netopiaPayments.ts...\n');
    
    // EXACT ca în cod:
    const isProduction = mockEnvironment.hostname === "lupulsicorbul.com" || 
        (mockEnvironment.hostname !== "localhost" &&
         !mockEnvironment.hostname.includes("netlify") &&
         !mockEnvironment.hostname.includes("preview"));

    const liveSignature = mockEnvironment.VITE_NETOPIA_SIGNATURE_LIVE || mockEnvironment.VITE_PAYMENT_LIVE_KEY;
