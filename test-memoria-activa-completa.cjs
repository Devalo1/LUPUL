...
const { UserProfileManager, extractInfoFromMessage } = require('./lib/firebase-user-profiles.cjs');
...
async function testMemoriaActivaCompleta() {
  ...
  try {
    ...
    const contextualQuestion = await profileManager.generateContextualQuestion();
