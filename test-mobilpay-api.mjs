  ...
  try {
    ...
    if (response.ok) {
      if (contentType && contentType.includes("application/json")) {
        ...
        return { success: true, type: "html", text: text.substring(0, 500) + "..." };
      }
    } else {
      // Eroare HTTP
...
async function runAllTests() {
  ...
  // Test production
  results.production = await testMobilPayEndpoint("production", ENDPOINTS.production);
  ...
  if (results.production.success) {
    ...
    console.log("⚠️  Doar sandbox funcționează - verifică credențialele production");
  } else {
    console.log("❌ Ambele endpoint-uri eșuează - verifică payload-ul și credențialele");
  }
