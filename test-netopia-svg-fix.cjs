...
function testDebugEndpoint() {
  return new Promise((resolve, reject) => {
    const url = 'https://lupul-si-corbul.netlify.app/.netlify/functions/netopia-debug';
    ...
    https.get(url, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log(`   Mode: ${jsonData.netopiaConfig?.mode || 'Unknown'}`);
          console.log(`   Endpoint: ${jsonData.netopiaConfig?.endpoint || 'Unknown'}`);
          console.log(`   Message: ${jsonData.message || 'No message'}\n`);
          
          if (jsonData.netopiaConfig?.mode === 'LIVE') {
            console.log('✅ LIVE mode is active - SVG redirect should be fixed!');
          } else {
      ...
      res.on('end', () => {
        ...
        if (data.includes('<html') || data.includes('<!doctype')) {
          ...
          if (data.includes('card.svg') || data.includes('wp-content')) {
            console.log('❌ Still contains SVG references - fix may not be complete');
          } else {
            console.log('✅ No SVG references found - redirect issue resolved!');
          }
          ...
          try {
            ...
            console.log('❌ Unexpected response format:', data.substring(0, 200) + '...');
          }
        }
...
async function runTests() {
  try {
    ...
    console.log('\n🎉 Test completed!');
    console.log('\n📋 Summary:');
    console.log('   - If you see "LIVE mode is active" and "LIVE Netopia endpoint"');
