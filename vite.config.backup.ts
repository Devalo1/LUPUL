...
import { emotionRuntimeFixPlugin } from "./plugins/emotion-runtime-fix-plugin";
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      ...
      // Plugin pentru copierea fișierelor de configurare Netlify
      {
        name: "netlify-config-copy",
        generateBundle() {
          // Copiem automat fișierele de configurare în dist
          this.emitFile({
            type: "asset",
            fileName: "_redirects",
            source: `# API routes
...
# Static assets - nu redirecționa fișierele statice!
...
# Doar pentru rutele SPA (fără extensie) redirecționează către index.html
/* /index.html 200`
          });
          ...
          this.emitFile({
            type: "asset",
            fileName: "_headers",
            source: `# CRITICAL: Headers pentru a fixa problema MIME type cu fișierele Emotion
